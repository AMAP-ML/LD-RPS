...
args = create_args()
os.makedirs(args.save_prompt_dir, exist_ok=True)
# If you have an 80G A100 GPU, you can put the entire model on a single GPU.
# Otherwise, you need to load a model using multiple GPUs, please refer to the `Multiple GPUs` section.
path = '/mnt/workspace/common/models/InternVL-Chat-V1-5'
...
for file_name in os.listdir(args.sample_dir):
    ...
    pixel_values = load_image(lq_path, max_num=12).to(torch.bfloat16).cuda()
    generation_config = dict(max_new_tokens=1024, do_sample=True)
    response = model.chat(tokenizer, pixel_values, question, generation_config)
    print(f'Filename: {file_name}\nAssistant: {response}')
    prompt_path = os.path.join(args.save_prompt_dir, file_name.split('.')[0]+'.txt')
