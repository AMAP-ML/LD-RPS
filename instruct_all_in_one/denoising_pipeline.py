...
def create_args():
    ...
    parser.add_argument("--degradation_para", type=list, default=[3.0, 0.002, 0.001])
    parser.add_argument("--guidance_scale", type=int, default=40000)
    parser.add_argument("--iter", type=int, default=1)
    parser.add_argument("--result_dir", type=str, default='./results/denoising')
    parser.add_argument("--sample_dir", type=str, default='./data/Kodak/input')
    parser.add_argument("--text_dir", type=str, default='./data/Kodak/prompt')
...
def get_latent_space(img, pipe):
    ...
    '''
    latents = 0.18215 * pipe.vae.encode(img).latent_dist.mean
...
def general_cond_fn(x, t, lq_latent=None, lq_image=None):
    with torch.enable_grad():
        ...
        """ with torch.no_grad():
            ...
            image.save('learned_lq_hazing.png') """
        ...
        pse = 0#perceptual_loss(x_in_lq[:args.batch_size, :3],
                                #x_tg_lq[:args.batch_size, :3].detach()) * 5e-4 if t < 700 else 0
        adv = 0#adversarial_loss((x_in_hq - x_tg_lq.detach())[:args.batch_size],
                                #(x_in_hq - x_in_lq).detach()) * 5e-5 if t < 700 else 0
        exp = 0#exp_loss(x_in_hq[:args.batch_size]) * 1e-5 if t < 100 else 0
        col = 0#col_loss(x_in_hq[:args.batch_size]) * 1e-5 if t < 100 else 0

        #if t < 700: mse, pse = condition_clamp(mse, pse)
...
def training(timesteps, pipe, latents, cond_fn, text_embeddings):
    ...
    for i, t in enumerate(timesteps):
        ...
        with torch.no_grad():
            ...
            image = (image[0] * 255).astype(np.uint8)
            image = Image.fromarray(image)
            image.save('pred_x0_denoise.png')
...
adversarial_loss = GANLoss(lr=1e-3).to(device)

if args.use_bn:
    degeneration_model = GenerativeDegradation_res(input_para=args.degradation_para, lr=1e-4).to(device)
else:
    degeneration_model = GenerativeDegradation(input_para=args.degradation_para, lr=1e-4).to(device)

exp_loss = L_exp(64, 0.5).to(device)
col_loss = L_color().to(device)

model_id = "stabilityai/stable-diffusion-2-1-base"
pipe = StableDiffusionPipeline.from_pretrained(model_id).to(device)
img_pipe = StableDiffusionImg2ImgPipeline.from_pretrained(model_id).to(device)
...
for file_name in os.listdir(args.sample_dir):
    ...
    latents = torch.randn((2, 4, 64, 64), device=device, generator=generator)
    ...
    for i in range(args.iter):
        ...
        latents1 = img_pipe.scheduler.add_noise(latents[0:1], noise[0:1], latent_timestep)
        latents2 = img_pipe.scheduler.add_noise(latents[1:2], noise[1:2], latent_timestep)
        latents = torch.cat([latents1, latents2])
